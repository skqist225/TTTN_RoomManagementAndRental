...
export default function initComp(user, room) {
...
async function restoreRoomImages(inputUploadPhotos, user, room) {
    isUploaded = true;
    ...
    allRoomImages
        ...
        .forEach(image => formData.append("roomImages", image));
    ...
    if (roomImages) {
        const filesArrPromise = roomImages.map(e => {
        ...
        const indexOfThumbnail = filesArr.findIndex(
            ({ name }) => name === room.thumbnail.split("/").pop()
        );
...
const appendedPhotosMapFirstTime = new Map();
function doPreviewImage(files, subPhotosContainer) {
...
export function doPreviewImageSecondTime(files, subPhotosContainer) {
    const defer = $.Deferred();
    let count = 0;
...
function readURL(files, inputUploadPhotos) {
...
export function addEmptyImage(files, uploadPhotos, subImagesContainer) {
    if (files.length - 1 < 4) {
...
export async function uploadImagesToFolder(user, room) {
    if (photos.length < 5) {
