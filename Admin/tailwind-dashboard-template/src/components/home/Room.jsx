...
export const Room = ({ room, index }) => {
    ...
    function changeImage(self, className) {
        ...
        const functionName = self.data("function-name");
        const roomId = self.data("room-id");

        let roomImages = roomId !== undefined ? $(`.${className}${roomId}`) : $(`.${className}`);
            ...
            <Link to={"/rooms/" + room.id}>
                ...
                >
                    <div className='flex items-center'>
                        <Image src={getImage("/svg/star.svg")} size='15px' />
                        <span>
                            {room.averageRatings}({room.numberOfReviews})
                        </span>
                    </div>
                ...
                <div className='room__name w-full'>{room.name}</div>
            </Link>
        </div>
    );
};
