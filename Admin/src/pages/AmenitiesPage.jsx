...
const AmenitiesPage = () => {
  ...
  const [amtCategory, setAmtCategory] = useState(0)
  ...
  if (amenities.length) {
    amenities = amenities.map(amenity => ({
      ...amenity, amtCategory: amenity.amentityCategory ? amenity.amentityCategory.name : 'None',
    }))
  }
  ...
  const roomColumns = [{
    ...
    title: 'Icon',
    field: 'iconImagePath',
    render: rowData => (<div className="normal-flex" style={{ width: '300px' }}>
    ...
    title: 'Category', field: 'amtCategory', defaultGroupOrder: 0,
  }, {
    title: '', field: '', render: rowData => (<div>
  ...
  async function restoreImage (folderName, imageName) {
    ...
    if (filesArr[0].name.includes('svg')) {
      $('#previewImage').attr('src', `http://localhost:8080/amentity_images/${imageName}`)
    } else {
  ...
  const handleSubmit = () => {
    ...
    console.log(name, amtCategory, description, type, image)
  ...
  useEffect(() => {
    if (amenity) {
      ...
      if (amenity.amentityCategory) {
        setAmtCategory(amenity.amentityCategory)
      }
      restoreImage('amentity_images', amenity.iconImage)
      setEditMode(true)
    }
  }, [amenity])
  ...
  return (<>
    <MaterialTable
      ...
      options={{
        headerStyle: {
          borderBottomColor: 'red', borderBottomWidth: '3px', fontFamily: 'verdana',
        },
