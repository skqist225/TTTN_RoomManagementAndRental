...
import { FormControl, InputLabel, MenuItem, Select, TextareaAutosize, TextField, useTheme, } from '@material-ui/core'
...
const EditRoomContent = ({ room }) => {
  ...
  useEffect(() => {
    if (images && images.length) {
      const rroom = {
        roomImages: images.map(image => image.split('/').pop()), username: user.email, folderno: room.id,
      }
  ...
  const [info, setInfo] = useState({
    bedroomCount: bedroom, bathroomCount: bathroom, accomodatesCount: accomodates, bedCount: bed,
  })
  ...
  const { updateRoomAction: { successMessage: uraSuccessMessage } } = useSelector(roomState)
  ...
  const {
    addUserAction: { successMessage, errorMessage }, user,
        ...
        <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
          ...
          <Box sx={{ width: '100%', marginTop: '20px' }}>
            ...
            {activeStep === steps.length ? (<React.Fragment>
              ...
              {activeStep === 0 && (<div className="flex items-center">
                      ...
                      <div className="mb-5">
                        <FormControl fullWidth required>
                          ...
                          <Select
                            ...
                            onChange={handleChange}
                          >
                            {privacies.map(privacy => (<MenuItem value={privacy.id}>
                              {privacy.name}
                            </MenuItem>))}
                          </Select>
                        </FormControl>
                      ...
                      <div className="mb-5">
                        <FormControl fullWidth required>
                          ...
                          <Select
                            ...
                            onChange={handleChange}
                          >
                            {amenitiesLst.map(amenity => (<MenuItem
                              value={amenity.id}
                              key={amenity.id}
                              style={getStyles(amenity.name, amenities, theme)}
                            >
                              {amenity.name}
                            </MenuItem>))}
                          </Select>
                          ...
                          <div className="mb-5">
                            <FormControl fullWidth required>
                              ...
                              <Select
                                ...
                                onChange={handleChange}
                              >
                                {rulesArray.map(rule => (<MenuItem
                                  value={rule.id}
                                  key={rule.id}
                                  style={getStyles(rule.title, rules, theme)}
                                >
                                  {rule.title}
                                </MenuItem>))}
                              </Select>
              ...
              {activeStep === 2 && (<div className="flex justify-center">
