...
public interface BookingDetailRepository extends JpaRepository<BookingDetail, Integer>, JpaSpecificationExecutor<BookingDetail> {
    ...
    @Query("SELECT b FROM BookingDetail b WHERE b.room.id = :roomId AND b.booking.state = 'APPROVED'")
    public List<BookingDetail> getBookedDates(Integer roomId);
    ...
    @Query("SELECT b FROM BookingDetail b WHERE b.booking.customer.id = :customerId AND CONCAT(b.room.name, ' ', b.booking.customer.firstName, ' ', b.booking.customer.lastName) LIKE %:query% ORDER BY b.bookingDate DESC")
    ...
    @Query("SELECT b FROM BookingDetail b WHERE b.booking.customer.id = :customerId"
            + " AND CONCAT(b.id, ' ', b.room.name, ' ', b.booking.customer.firstName, ' ', b.booking.customer.lastName) LIKE %:query% ORDER BY b.bookingDate DESC")
    public List<BookingDetail> getBookedRoomsByUser(Integer customerId, String query);

    @Query("SELECT b FROM BookingDetail b WHERE b.room.id IN (:roomIds) AND b.bookingDate >= :startDate AND b.bookingDate <= :endDate")
...
//    @Query("SELECT b FROM BookingDetail b WHERE b.room.id IN (:roomIds) AND b.room.name LIKE %:query% AND b.bookingDate <= :bookingDate AND b.bookingDate >= :BookingDetailDate2 AND b.booking.state IN (:states)")
//    public Page<BookingDetail> getBookingDetailsByRooms(Integer[] roomIds, String query, List<State> states,
//                                            LocalDateTime bookingDate, LocalDateTime BookingDetailDate2,
//                                            Float totalFee,
//                                            Pageable pageable);

//    @Query("SELECT b FROM BookingDetail b"
//            + " WHERE b.room.id IN (:roomIds) AND b.room.name LIKE %:query% AND b.bookingDate <= :bookingDate"
//            + " AND b.bookingDate >= :BookingDetailDate2"
...
//                                               Pageable pageable);
//
//    @Query("SELECT b FROM BookingDetail b WHERE b.room.id IN (:roomIds) AND b.room.name LIKE %:query% AND year(b.bookingDate)=:year AND month(b.bookingDate)=:month AND b.booking.state IN (:states) ORDER BY b.bookingDate ASC")
...
//
//    @Query("SELECT b"
//            + " FROM BookingDetail b WHERE b.room.id IN (:roomIds) AND b.room.name LIKE %:query% AND b.booking.state = :state ORDER BY b.bookingDate DESC")
...
//
//    @Query("SELECT b FROM BookingDetail b WHERE b.room.id IN (:roomIds) AND b.id = :BookingDetailId ORDER BY b.bookingDate DESC")
