...
//		BookingListResponse bookings = bookingDetailService.getBookingListByRooms(roomIds, filters, page);
    ...
    public ResponseEntity<StandardJSONResponse<String>> userCancelBooking(@PathVariable("bookingid") Integer bookingid,
                                                                          @AuthenticationPrincipal UserDetailsImpl userDetailsImpl) throws MessagingException {
        try {
            User customer = userDetailsImpl.getUser();
            Booking booking;
            try {
    ...
    public ResponseEntity<StandardJSONResponse<List<BookingUserOrderDTO>>> fetchUserOrders(
            @AuthenticationPrincipal UserDetailsImpl userDetailsImpl) {
        User customer = userDetailsImpl.getUser();
        List<BookingUserOrderDTO> bookings = bookingService.findByCustomerAndStatus(customer, Status.CART).stream().map(booking -> BookingUserOrderDTO.build(booking)).collect(Collectors.toList());

        return new OkResponse<List<BookingUserOrderDTO>>(bookings).response();
    }
}
